...
- `src/styles/files/tokens.css` — CSS custom properties (`--rds--*`)
- `src/styles/files/_variables.scss` — SCSS variables (for `@media` queries — CSS custom properties don't work there)
- `src/styles/files/base-styles.{css,scss}` — generated element styles + layout classes
- `src/styles/files/layout.css`, `typography.css` — generated stylesheets
- `dist/cutheme/theme-rds.json` and `dist/cutheme/integrate.php` — WordPress theme.json + integration helper

Run `pnpm c2b` to regenerate any of the above. Files in `src/styles/files/` are never edited by hand.
...
1. **Asset generation** ([`scripts/build-assets.mjs`](../../scripts/build-assets.mjs)) — produces [`src/config/assets.ts`](../../src/config/assets.ts) and `src/styles/files/_assets.scss` from [`src/config/assets.json`](../../src/config/assets.json).
